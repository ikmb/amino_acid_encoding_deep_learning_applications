...
@see RNNBluePrint.py  
@note: The model uses BLOSUM62 Matrix to encode the provided amino acids
"""
# load the modules:
...
## load BLOSUM matrix
raw_blosum=pd.read_csv('BLOSUM.csv')
# prepare BLOSUM matrix
blosum=np.zeros((21,20),dtype=np.float32)
count=0
for index, char in enumerate(tokenizer.word_index):
    count+=1;
    blosum[index+1,:]= np.array(raw_blosum.loc[raw_blosum.AA==char.upper()])[:,1:21].reshape(20,)
    if count ==20:
        break    
...
## create the model and assign embedding weights to it
model=buildHlaRNNModel(inputSize=37,numberOfTokens=21,
                           embDim=20,embWeights=blosum,trainEmb=False) # use BLOSUM for encoding and prevent weight updates
