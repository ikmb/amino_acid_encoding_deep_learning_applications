...
@details Constructing a DPPI models with a user defined parameters, train it on a specific fraction of input data 
        using a specific GPU and then write the results dict as a pickle object on a user defined object.
...
import pickle
import os
from DPPIBluePrint import DPPIModelOneHot
import argparse
...
model=DPPIModelOneHot()
# encode the inputs Tensors into one-hot encoding:
# Training metrics: 
...
InputSignature=[
         tf.TensorSpec(shape=(None,firstProteinArrayTrain.shape[1],numTokens), 
                       dtype=tf.float32),
         tf.TensorSpec(shape=(None,firstProteinArrayTrain.shape[1],numTokens), 
                       dtype=tf.float32),
...
def trainStep(inputATensor,inputBTensor,label):
    ...
    @param inputATensor: tensor, a rank three tensor of shape (batch_size, seq_len, depth) which contain 
    the tokenized sequences of the first protein. 
    @param inputBTensor: tensor, a rank three tensor of shape (batch_size, seq_len, depth) which contain 
...
def trainEpoch(numEpoch,outputPath):
    ...
    # define metric variables to store the results: 
         ...
         for (batch,(inputTensorA,inputTensorB,labels)) in enumerate(trainDataset):
             inputTensorA=tf.one_hot(inputTensorA,depth=numTokens,dtype=tf.float32)
             inputTensorB=tf.one_hot(inputTensorB,depth=numTokens,dtype=tf.float32)
